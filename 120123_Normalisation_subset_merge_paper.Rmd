...
```{r 003, load10x adapted with project, message=FALSE,echo=FALSE}
#to name project 
...
```{r 004, data loading, message=FALSE,echo=FALSE}
...
# Sample name from the paper:
spat.data_A (A2_OB_virgin20) = Virgin F_1
spat.data_B (B2_OB_mother20) = Mother at perinatal care
spat.data_C (C2_OB_virgin30) = Virgin F_2
spat.data_D (D2_OB_mother30) = Mother at Peri-weaning

# Quality control
Addapted from https://ludvigla.github.io/STUtility_web_site/Quality_Control.html



```{r 005, data loading, message=FALSE,echo=FALSE}
p1 <- ggplot() +
...
```{r 006, data loading, message=FALSE,echo=FALSE}
...
With this we also can see the region that were folded over each other.

```{r 007, message=FALSE,echo=FALSE}
...
this is to facilitate the subseting and remove the connective tissue 

```{r 008, normalization, warning=FALSE, message=FALSE, message=FALSE,echo=FALSE,width = 8,fig.height = 10}
...
# save(spat.data_A, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_spat.data_A.RData")
# save(spat.data_B, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.8_spat.data_B.RData")
# save(spat.data_C, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_spat.data_C.RData")
# save(spat.data_D, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_spat.data_D.RData")

SpatialDimPlot(spat.data_A)#5 6 10
...
SpatialDimPlot(spat.data_C)# 3 and 5
...
SpatialDimPlot(spat.data_D)# 4 and ev 5
...
From Seurat: https://satijalab.org/seurat/articles/spatial_vignette.html
In order to work with multiple slices in the same Seurat object, we used the merge function.
This then enables joint dimensional reduction and clustering on the underlying RNA expression data.

```{r 009, subset, warning=FALSE, message=FALSE, message=FALSE,echo=FALSE,width = 8,fig.height = 10}
...
subset_D <- subset(spat.data_D, idents = c(0, 1, 2, 3,5,6,7,8,9,10)) #5 as well since overlaping # not needed since will counts correct
...
# save(subset_A, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_subset_A.RData")
# save(subset_B, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_subset_B.RData")
# save(subset_C, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_subset_C.RData")
# save(subset_D, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_subset_D.RData")
```

# Merge
Cells were merged and normalized with SCT on the spatial assay
used 20 dims based on the ElbowPlot and for finding clusters with 0.5



```{r 010, normalization, warning=FALSE, message=FALSE, message=FALSE,echo=FALSE}
...
# save(spat.merge.all, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_sct_spat.merge.all.RData")
...
```{r 011, dimreduction,  message=FALSE, echo=FALSE,width = 10,fig.height = 10}
...
```{r 012, dimreduction,  message=FALSE, echo=FALSE,width = 8,fig.height = 10}
...
```{r 013, dimreduction,  message=FALSE, echo=FALSE}
...
# DimPlot(subset_OB_merge, reduction = "umap", group.by = c("ident"),label = TRUE)
# DimPlot(subset_OB_merge, reduction = "umap", split.by = ("orig.ident"))
# DimPlot(subset_OB_merge, reduction = "umap", group.by = c("ident"), split.by = ("orig.ident"))


```
#Save Data

```{r 014, dimreduction,  message=FALSE, echo=FALSE}
# 
# spat.merge.all_01_23 <- spat.merge.all
#save(spat.merge.all_01_23, file = "/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/Reproduce_paper/spat.merge.all_01_23.RData")

# load("/media/Jonas_Kretz/Visium_gene_expression/Visium_mouse_brain/Analysis_mouse_brain/Workflow_sct_merge_log/Rmd_and_data/processed_data/0.5_sct_spat.merge.all.RData")
# 
# DimPlot(spat.merge.all, reduction = "umap", group.by = c("ident"),label = TRUE)
# DimPlot(spat.merge.all_new, reduction = "umap", group.by = c("ident"),label = TRUE)
```
